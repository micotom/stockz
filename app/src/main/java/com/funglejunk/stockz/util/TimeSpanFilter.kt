...
sealed class TimeSpanFilter {
    object Max : TimeSpanFilter() {
        override fun invoke(dataPoint: RepoHistoryData.Data): Boolean = true
        override val startDate: LocalDate = now.minusYears(3L)
    }
    object Year : TimeSpanFilter() {
        override fun invoke(dataPoint: RepoHistoryData.Data): Boolean =
    ...
    abstract operator fun invoke(dataPoint: RepoHistoryData.Data): Boolean
